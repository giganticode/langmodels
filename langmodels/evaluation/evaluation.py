...
def _evaluate_model_on_line(model: TrainedModel, line: str, extension: str,
                            ...
                            token_categories: Optional[Set[TokenCategory]],
    ...
    for metric_name in metrics:
        ...
        for evaluation_customization, evaluation_result in metric(model, line, extension, append_eof, token_categories, context_modification).items():
...
def evaluate_model_on_string(model: TrainedModel, text: str, extension='java',
    ...
    for i, line in enumerate(line_iterable):
        ...
        line_evaluation = _evaluate_model_on_line(model, line, extension, metrics, token_categories, append_eof and last_line, context_modification=context_modification)
...
def evaluate_model_on_file(model: TrainedModel, file: Path, metrics: Optional[Set[str]] = None,
    ...
    return evaluate_model_on_string(model, text, suffix, metrics, token_categories,
...
def evaluate_model_on_project_set(model: TrainedModel, path: Path, metrics: Optional[Set[str]] = None,
                                  token_categories: Optional[Set[TokenCategory]] = None, max_context_allowed: int = sys.maxsize) \
    ...
    for directory in dirs:
        ...
        result[directory] = evaluate_model_on_path(model, Path(os.path.join(root, directory)),
                                                   metrics, token_categories, max_context_allowed)
    return result
...
def evaluate_model_on_path(model: TrainedModel, path: Path, metrics: Optional[Set[str]] = None,
                           token_categories: Optional[Set[TokenCategory]] = None, context_modification: Optional[ContextModification] = None) \
    ...
    for file in t:
        ...
        evaluation: Evaluation = evaluate_model_on_file(model, file, metrics,
                                                        token_categories, result_per_line=False, context_modification=context_modification)
