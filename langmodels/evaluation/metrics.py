...
from langmodels.evaluation.common import FullWordIterator, to_full_token_string, FilteringTokenIterator, \
    CorpusFilterOptions
...
def word_average(subword_entropies: List[float],
                 metadata: PreprocessingMetadata,
                 filter_options: CorpusFilterOptions) -> float:
    """
    >>> word_average([], PreprocessingMetadata(set(), [0]), CorpusFilterOptions())
    0.0

    >>> word_average([1.0, 2.0, 3.0], PreprocessingMetadata(set(), [0, 1, 3]), CorpusFilterOptions())
    3.0
    """
    word_entropies = [we for we in FilteringTokenIterator(subword_entropies, metadata,
                                                          filter_options, agg=sum)]
...
def bin_entropy(model: TrainedModel,
                ...
                filter_options: CorpusFilterOptions = CorpusFilterOptions()) -> Tuple[List[float], float]:
    """
    Changes the state of the model!
    """
    entropies = model.get_entropies_for_prep_text(prep_line)
    aggregated_entropy = word_average(entropies, prep_metadata, filter_options)
    return entropies, aggregated_entropy


def full_token_mrr(model: TrainedModel,
                   prep_line: List[str],
                   metadata: PreprocessingMetadata,
                   filter_options: CorpusFilterOptions = CorpusFilterOptions()) -> Tuple[List[int], float]:
    """
    Changes the state of the model!
    """
    inverse_rank_sum = .0
    count = 0
    results = []
    for full_token in FilteringTokenIterator(prep_line, metadata, filter_options):
